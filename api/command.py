...
from exabgp.reactor.NLRIvar import *
...
def show_routes (self, reactor, service, command):
	...
	reactor.plan(callback())
	return True
def show_routes_nlri (self, reactor, service, command):
	def callback ():
		NU=NLRI_Update()
		reactor.answer(service,'%s' % NU.recive_nlri)
		yield True

	reactor.plan(callback())
	return True
def show_routes_withdraw_nlri (self, reactor, service, command):
	def callback ():
		NU=NLRI_Update()
		reactor.answer(service,'%s' % NU.recive_nlri_withdraw)
		yield True
...
def announce_route (self, reactor, service, command):
	def callback (self, command, nexthops):
		...
		if not changes:
			...
			reactor.answer(service,"Command could not parse route in : %s" % command,'warning')  #eason
			yield True
		else:
			peers = []
...
def announce_attribute (self, reactor, service, command):
	def callback (self, command, nexthops):
		...
		if not changes:
			...
			reactor.answer(service,"Command could not parse route in : %s" % command)  #eason
			yield True
		else:
